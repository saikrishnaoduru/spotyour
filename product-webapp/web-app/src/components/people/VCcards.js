...
const VcCards = (props, datetopass) => {
  ...
  const getSlotAvailability = (queryParameter) => {
    ...
    let url = "https://spotyourvaccine.stackroute.io/vaccination-center-service/api/v1/slots/available/" + queryParameter;
    return fetch(url,
      {
        headers: {
  ...
  const UserInformationByEmailId = (emailId) => {
    ...
    let url = "https://spotyourvaccine.stackroute.io/user-service/api/v1/user/user/" + emailId;
    return fetch(url,
      {
        headers: {
          'Content-Type': 'application/json',
